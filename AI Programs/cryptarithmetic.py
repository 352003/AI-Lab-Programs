def isSolvable(words, result):
    ...
    mp = [-1] * 26
    if solve(words, 0, 0, mp, used, Hash, CharAtfront):
        return "Yes"
    else:
        return "No"
...
def solve(words, i, S, mp, used, Hash, CharAtfront):
    ...
    ch = words[i]
    val = mp[ord(ch[0]) - ord('A')]

    if val != -1:
        return solve(words, i + 1, S + val * Hash[ord(ch[0]) - ord('A')], mp, used, Hash, CharAtfront)
